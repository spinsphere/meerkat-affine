...
test('should be able to emit events and send notifications', async t => {
...

  {
    ...
    const requestRequestedNotification = app.queue.last(
      'notification.sendInvitationReviewRequested'
    );
    t.truthy(requestRequestedNotification);
    // find admin
    const admins = await t.context.models.workspaceUser.getAdmins(tws.id);
    t.deepEqual(
      requestRequestedNotification.payload,
      {
        inviteId: memberInvite.inviteId,
        reviewerId: admins[0].id,
      },
      'should send review requested notification'
    );
