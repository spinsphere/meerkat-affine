...
declare global {
  interface Jobs {
    ...
    'notification.sendInvitationReviewRequested': {
      reviewerId: string;
      inviteId: string;
    };
...
export class NotificationJob {
  ...
  @OnJob('notification.sendInvitationReviewRequested')
  async sendInvitationReviewRequested({
    reviewerId,
    inviteId,
  }: Jobs['notification.sendInvitationReviewRequested']) {
    const invite = await this.models.workspaceUser.getById(inviteId);
    if (!invite) {
      return;
    }
    await this.service.createInvitationReviewRequested({
      userId: reviewerId,
      body: {
        workspaceId: invite.workspaceId,
        createdByUserId: invite.userId,
